...
// Writes screenshots/<name>.png (1440x900) and screenshots/thumbs/<name>.png (640x400) — <name> is owner--repo for a repo
// outside the catalogued org (see screenshotName in build-data.mjs) —
...
import { screenshotName } from './build-data.mjs';
...
async function main() {
  ...
  await Promise.all(Array.from({ length: Math.min(CONCURRENCY, targets.length) }, async () => {
    while (i < targets.length) {
      const t = targets[i++];
      results.push(await shoot(browser, t.p, t.url, screenshotName(t.p, data.org)));
    }
  }));
  await browser.close();

  const stem = p => screenshotName(p, data.org);
  for (const r of results) {
    const p = data.projects.find(x => (x.full_name || x.name) === r.key);
    if (!p) continue;
    if (r.ok) { p.screenshot = `screenshots/${stem(p)}.png`; p.screenshot_source = 'live'; p.screenshot_note = `Captured from ${r.url} at 1440x900 on ${r.at.slice(0, 16).replace('T', ' ')} UTC`; }
    else if (existsSync(path.join(OUT, `${stem(p)}.png`))) { p.screenshot = `screenshots/${stem(p)}.png`; if (!p.screenshot_source || p.screenshot_source === 'none') p.screenshot_source = 'live'; p.screenshot_note = `Kept previous screenshot; ${r.reason}`; }
  ...
  for (const p of data.projects) {
    ...
    if (existsSync(path.join(OUT, `${stem(p)}.png`))) { p.screenshot = `screenshots/${stem(p)}.png`; if (!p.screenshot_source || p.screenshot_source === 'none') p.screenshot_source = 'local-render'; }
...
async function shoot(browser, p, url, stem) {
  const at = new Date().toISOString(), key = p.full_name || p.name;
  const live = await checkLive(url, { timeoutMs: 15000 });
  if (!live.live) return { name: p.name, key, url, ok: false, at, reason: `URL not live (${live.status ?? live.error})` };
  let ctx;
  try {
    ...
    await page.screenshot({ path: path.join(OUT, `${stem}.png`), type: 'png' });
    ...
    try {
      ...
      await tpage.screenshot({ path: path.join(THUMBS, `${stem}.png`), type: 'png' });
    } finally { await tctx.close(); }
    console.log(`  ✓ ${p.name} ← ${url}`);
    return { name: p.name, key, url, ok: true, at };
  } catch (err) {
    console.log(`  ✗ ${p.name}: ${err.message.split('\n')[0]}`);
    return { name: p.name, key, url, ok: false, at, reason: `capture failed: ${err.message.split('\n')[0].slice(0, 160)}` };
  } finally {
